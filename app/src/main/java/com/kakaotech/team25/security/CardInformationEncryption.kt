...
class CardInformationEncryption {
    ...
    fun encryptCBC(card: CardInfor): String {
        val str = "cardNo=${card.getCardNumber()}&expYear=${card.getExpiredDate().substring(
            2,
            4,
        )}&expMonth=${card.getExpiredDate().substring(
            0,
            2,
        )}&idNo=${card.getBirth()}&cardPw=${card.getPassword()}" // 암호화할 문자열
